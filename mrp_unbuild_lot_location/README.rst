...
When a manufacturing order is selected on an unbuild order, this module
automatically sets the source location based on where the lot/serial
number of the finished product is currently stocked.

This prevents inventory discrepancies that could occur when the default
warehouse stock location is used instead of the actual storage location.
